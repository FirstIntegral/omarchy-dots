...
BarWidget {
  ...
  // Show all icons inline - no overflow drawer/arrow
  readonly property var pinnedItems: {
    var all = bucket("all")
    var result = []
    for (var i = 0; i < all.length; i++) {
      var iid = String(all[i].id || "")
      if (hiddenIds.indexOf(iid) === -1) result.push(all[i])
    }
    return result
  }
  readonly property var drawerItems: []
  readonly property var allItems: bucket("all")
  readonly property int drawerCount: 0
  ...
  Component {
    ...
    Item {
      ...
      readonly property int drawerBlockWidth: 0
      ...
      // Right-click empty tray area to manage hidden icons (replaces old expandIcon right-click)
      MouseArea {
        anchors.fill: parent
  ...
  Component {
    ...
    Item {
      ...
      readonly property int drawerBlockHeight: 0
